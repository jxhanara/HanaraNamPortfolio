...
/**
 * Free × Free scroll-driven dual-phone walkthrough.
 *
 * Same scroll-pin / wheel-snap mechanics as the other two tabs, but the phones
 * hand off rather than coexist: John (left) plays steps 1–3 alone, his phone
 * then disappears, and Jennifer (right) plays steps 4–5 alone. Each step seeks
 * its driving video to its entry state and plays one segment, pausing at its
 * boundary so the phone stays in sync with the narrative.
 */
export function PrototypeFreeFreeWalkthrough() {
  const steps = bumbleFreeFreeDualPhone.steps;
  ...
  const johnVisibleRef = useRef(true);
  ...
  useEffect(() => {
    johnVisibleRef.current = johnVisible;
  }, [johnVisible]);
  ...
  const applyStep = useCallback(
    (i: number) => {
      ...
      if (step.johnVisible !== johnVisibleRef.current) {
        johnVisibleRef.current = step.johnVisible;
        setJohnVisible(step.johnVisible);
      }
      if (john) {
        if (step.johnVisible && step.johnPlay) {
  ...
  useEffect(() => {
    ...
    const syncPlaybackForView = (inView: boolean) => {
      ...
      if (inView) {
        if (step.johnVisible && step.johnPlay) {
          resumeWithinSegment(john, step.johnPauseAt);
        }
        if (step.jenniferVisible && step.jenniferPlay) {
          ...
          <div className={dp.walkthroughGrid}>
            ...
            <div className={dp.phonesWrap}>
              <div className={dp.phonesRow}>
                <PhoneFrame
                  label={bumbleFreeFreeDualPhone.johnLabel}
                  videoRef={johnVideoRef}
                  src={bumbleFreeFreeDualPhone.johnVideoSrc}
                  visible={johnVisible}
                  ariaHiddenWhenInvisible
                />
                <PhoneFrame
                  label={bumbleFreeFreeDualPhone.jenniferLabel}
                  videoRef={jenniferVideoRef}
                  src={bumbleFreeFreeDualPhone.jenniferVideoSrc}
            ...
            <div className={dp.descriptionColumn}>
              ...
              <span
                ...
                aria-hidden
              >
                {bumbleFreeFreeDualPhone.scrollHint}
